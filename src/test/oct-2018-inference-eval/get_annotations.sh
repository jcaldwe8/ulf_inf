...
# Generated files.
ULF_SQL_REQUEST_FILE=${WORKING_DIR}"request-sents-n-ulfs-raw.sql"
...
INF_SQL_OUTPUT_FILE=${WORKING_DIR}"isid-inf-out-raw.tsv"
FINAL_INF_OUTPUT_FILE="isid-inf-out.tsv"
...
mysql -h ebdb-instance2.cuainl7oscs9.us-east-2.rds.amazonaws.com -P 3306 -u gkim21 -p ebdb --password < ${INF_SQL_REQUEST_FILE} > ${INF_SQL_OUTPUT_FILE}

python combine-and-format-ulf-anns.py ${ULF_SQL_OUTPUT_FILE} ${DATASET_JSON_FILE} ${TEMP_FINAL_ULF_FILE}
python combine-and-format-inf-anns.py ${INF_SQL_OUTPUT_FILE} ${TEMP_FINAL_INF_FILE}

cp ${TEMP_FINAL_ULF_FILE} ${FINAL_ULF_OUTPUT_FILE}
cp ${TEMP_FINAL_INF_FILE} ${FINAL_INF_OUTPUT_FILE}
